...
def _write_build_id():
    """Bake the commit hash into launcher/_build_id.py for the About tab.

    The file is generated, not tracked in git: a packaged build cannot ask git
    at runtime (the exe has no checkout next to it), so the hash has to be
    compiled in. A failed lookup still writes the file with an empty hash so
    the compiled import always resolves -- the About tab then shows the bare
    ...
    try:
        ...
        commit = ""
    path = os.path.join(ROOT, "launcher", "_build_id.py")
    with open(path, "w", encoding="utf-8") as handle:
        handle.write('"""Generated by build/build.py at build time; not tracked in git."""\n\n')
        handle.write("COMMIT = {!r}\n".format(commit))
